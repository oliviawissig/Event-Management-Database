...
class DB:
    ...
    def get_mysql_credentials(self, configfile, dbms):
        ...
        try:
            if configfile:
                ...
                self._transactions = config[dbms]['transactions']
        except:
    ...
    def getAcc(self, username):
        acc = self.select(query="""SELECT password, user FROM account WHERE username = %s""", values=username)
        return acc

    def getUser(self, userid):
        user = self.select(query="""SELECT name FROM user WHERE user_id = %s""", values=userid)
        return user

    def write_transaction(self, query, value, file):
        #query = DELETE FROM attendeeTicket WHERE user = %s;
        new_query = query.replace("%s", value)

        with open(file, 'a') as f:
            f.write(new_query + ";\n")

    def write_insert_transaction(self, table, attributes, values, file):
        query = None
        new_query = None
        if not query:
            wildcards = self.wildcars(len(values))
        ...
        index = 0
        for w in wildcards:
            new_query = query.replace("%s", values[index])
    ...
    def encrypt_passwords(self):
        #UPDATE account SET password = 'newPassword345&' WHERE account_id = 1;
        pwds = self.select(query="""SELECT password FROM account""")

        index = 1
        for p in pwds:
            user_pass = p[0]
            new_pass = hashlib.md5(user_pass.encode())
            query = """UPDATE account SET password = '{}' WHERE account_id = {}""".format(new_pass.hexdigest(), index)
            index += 1
            self.update(query)
